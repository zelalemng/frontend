...
const ServiceFormPopup = ({ addService }) => {
  ...
  useEffect(() => {
    // Fetch categories
    axios.get(`${process.env.REACT_APP_API_URL}/api/categories`) // Adjust the API endpoint as per your backend
      .then(response => {
        // Filter categories where status is 'active'
        const activeCategories = response.data.filter(cat => cat.category_status === 'active');
